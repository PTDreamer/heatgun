...
void write_data(uint8_t *data) {
	...
	HAL_I2C_Master_Transmit(m_i2c, m_oled_addr, idata, 129, 100);
#endif
}
void write_cmd(uint8_t data) {
	...
	HAL_GPIO_WritePin(OLED_CS_GPIO_Port, OLED_CS_Pin,GPIO_PIN_SET);//CS4
#else
	idata[0] = 0;
	idata[1] = data;
	HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(m_i2c, m_oled_addr, idata, 2, 100);
#endif
}
